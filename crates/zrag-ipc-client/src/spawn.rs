...
fn spawn_daemon(
    ...
    let (daemon_bin, needs_subcommand) = if exe_stem == "zebrarag" {
        (exe, true)
    } else {
        let zebrarag = dir.join("zebrarag");
        if zebrarag.exists() {
            (zebrarag, true)
        } else {
            let fallback = dir.join("zrag-daemon");
            if fallback.exists() {
                (fallback, false)
            } else {
                anyhow::bail!(
                    "neither zebrarag nor zrag-daemon found in {} — install one",
                    dir.display()
                );
            }
        }
    ...
    if let Some(key) = remote_api_key
        && let Some((provider, _)) = model.and_then(zrag_remote_embed::RemoteProvider::from_model_id)
    {
