...
use zrag_tree_sitter::{Language, detect_from_path, frontend_for};
...
fn all_lang_skip_dirs() -> Vec<&'static str> {
    ...
    for lang in [
        ...
        let cfg = zrag_tree_sitter::frontend_for(lang).config();
...
/// used by `zrag-pipeline::indexer::index_project` — the indexer has already
    ...
    F: Fn(u32),
{
    ...
    let mut all_symbols: Vec<zrag_ts_core::types::Symbol> = Vec::new();
    let mut all_edges: Vec<zrag_ts_core::types::Edge> = Vec::new();
...
fn build_qualified_map(
    symbols: &[zrag_ts_core::types::Symbol],
    files: &[FileEntry],
    root: &str,
) -> HashMap<String, u32> {
...
fn resolve_edges(
    edges: &mut [zrag_ts_core::types::Edge],
    files: &[FileEntry],
    ...
    for edge in edges.iter_mut() {
        let zrag_ts_core::types::Target::Unresolved(name) = &edge.to else {
            continue;
        };
        ...
        edge.to = match resolved {
            Some(id) => zrag_ts_core::types::Target::Resolved(id),
            None => zrag_ts_core::types::Target::External(format!("*{lookup}")),
        };
    }
}
...
fn build_reverse_edges(
    edges: &[zrag_ts_core::types::Edge],
) -> HashMap<u32, Vec<zrag_ts_core::types::Edge>> {
    let mut reverse: HashMap<u32, Vec<zrag_ts_core::types::Edge>> =
        HashMap::with_capacity(edges.len());
    for edge in edges {
        if let zrag_ts_core::types::Target::Resolved(target_id) = edge.to {
            let reverse_edge = zrag_ts_core::types::Edge {
                from: target_id,
                to: zrag_ts_core::types::Target::Resolved(edge.from),
                kind: edge.kind,
                line: edge.line,
            };
...
fn build_forward_edges(
    ...
    let mut forward: HashMap<u32, Vec<zrag_ts_core::types::Edge>> =
        HashMap::with_capacity(edges.len());
    for edge in edges {
...
pub fn files_by_language(files: &[FileEntry], lang: zrag_tree_sitter::Language) -> Vec<u16> {
    files
        .iter()
        .enumerate()
...
mod tests {
    use zrag_tree_sitter::Language;
    use zrag_ts_core::types::{Edge, EdgeKind, Kind, Symbol, Target};
    ...
    fn search_dep_candidates_show_resolvable_aliases() {
        ...
        let files = vec![
            file_entry(0, "/p/crates/zrag-dsl/src/index.rs"),
            file_entry(1, "/p/crates/zrag-store/src/chunks_table.rs"),
        ];
    ...
    fn qualified_map_adds_rust_workspace_crate_aliases() {
        ...
        let map = build_qualified_map(&symbols, &files, "/p");
        assert_eq!(map.get("zrag_dsl::index::build_index"), Some(&0));
        assert_eq!(
            map.get("zrag_store::chunks_table::ChunksTable::build_index"),
            Some(&1)
        );
    ...
    fn resolve_edges_rewrites_crate_relative_call() {
        ...
        let files = vec![
            file_entry(0, "/p/crates/zrag-pipeline/src/search.rs"),
            file_entry(1, "/p/crates/zrag-pipeline/src/fusion.rs"),
        ];
        let mut edges = vec![Edge {
            from: 0,
    ...
    fn resolve_edges_rewrites_self_and_super_calls() {
        ...
        let files = vec![
            file_entry(0, "/p/crates/zrag-pipeline/src/render/dsl.rs"),
            file_entry(1, "/p/crates/zrag-pipeline/src/render.rs"),
        ];
        let map = build_qualified_map(&symbols, &files, "/p");
    ...
    fn external_qualified_call_does_not_use_bare_fallback() {
        ...
        let files = vec![
            file_entry(0, "/p/crates/zrag-pipeline/src/search.rs"),
            file_entry(1, "/p/crates/zrag-pipeline/src/local.rs"),
        ];
        let mut edges = vec![Edge {
            from: 0,
    ...
    fn resolve_edges_method_call_uses_unique_bare_name() {
        ...
        let files = vec![
            file_entry(0, "/p/crates/zrag-pipeline/src/search.rs"),
            file_entry(1, "/p/crates/zrag-embed/src/lib.rs"),
        ];
        let mut edges = vec![Edge {
            from: 0,
    ...
    fn clean_renderer_keeps_external_call_leaf() {
        ...
        let files = vec![file_entry(0, "/p/crates/zrag-pipeline/src/search.rs")];
