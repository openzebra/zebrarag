...
use zrag_ann::{AnnCache, AnnHandle, AnnIndexBuilder, SearchMethod, SearchParams};
use zrag_common::file_type::FileType;
use zrag_embed::AnyEmbedEngine;
use zrag_rerank::TurboReranker;
use zrag_rerank::gpu::{
    BATCH_SIZE, GpuTurboScratch, TurboCodeBatch, TurboScorerCache, parse_turbo_code_into,
    score_batch,
};
use zrag_store::chunks_table::{ChunkHit, ChunksTable};
...
pub async fn search(
    ...
    let params = match params_override.or(parsed_params.as_ref()) {
        Some(params) => params,
        None => {
            fallback_params = zrag_ann::choose_method(
...
async fn rebuild(
    chunks: &zrag_store::chunks_table::ChunksTable,
    dim: usize,
    params: &SearchParams,
) -> Result<zrag_ann::AnnIndex> {
