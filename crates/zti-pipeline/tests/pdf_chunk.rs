...
//! inputs mirror what `zti_pdf::extract_pages` emits for the synthetic fixture
//! (validated in `zti-pdf/tests/extract.rs`), so this is effectively a
//! round-trip without re-parsing the PDF bytes here.
//!
//! Run with: `cargo test -p zti-pipeline --features testing`

#![cfg(feature = "testing")]

use zti_pdf::PageText;
use zti_pipeline::pdf_chunk::pack_pdf_pages;
use zti_pipeline::testing::assemble_pdf_contents;
...
// (see `zti-protocol/tests/pdf_render.rs`), not prepended into the chunk body —
