...
//!
//! These tests are `#[ignore]`'d and gated on `ZRAG_PDF_FIXTURE_DIR` so they
//! never run in CI (the 905 KB PDF is not redistributable and not committed).
//! Run locally with:
//!
//! ```sh
//! ZRAG_PDF_FIXTURE_DIR=/Users/hicaru/projects/zebra/test \
//!   cargo test -p zrag-pdf -- --ignored real_pdf
//! ```
//!
...
/// Resolve the first `*.pdf` under `ZRAG_PDF_FIXTURE_DIR`, or skip the test.
fn fixture_pdf() -> PathBuf {
    let dir = std::env::var("ZRAG_PDF_FIXTURE_DIR")
        .expect("set ZRAG_PDF_FIXTURE_DIR to the dir holding the Knuth PDF");
...
#[test]
#[ignore = "requires ZRAG_PDF_FIXTURE_DIR pointing at the Knuth PDF; not committed"]
...
#[test]
#[ignore = "requires ZRAG_PDF_FIXTURE_DIR; characterizes the fixed inter-word spacing"]
