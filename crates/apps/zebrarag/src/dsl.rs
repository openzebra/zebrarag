...
use zrag_dsl::DslChunker;
use zrag_dsl::chunking::{chunk_tabular_file, chunk_text_file};
use zrag_dsl::render::dsl::{DslRenderer, render_files_only};
use zrag_dsl::render::tree::AsciiTreeRenderer;
use zrag_tree_sitter::{frontend_for, parse_kinds, parse_language};
...
pub fn run_dsl(root: &Path, command: DslCommands) -> Result<()> {
    ...
    } = command
    {
        ...
        let (index, _text_files) = zrag_dsl::build_index(dep_root.to_string_lossy().as_ref())?;
        match zrag_dsl::resolve_name(&index, name_lib) {
            zrag_dsl::NameMatch::Found(id) => {
                print!(
                    "{}",
                    zrag_dsl::render_symbol_overview(&index, id, depth, 6000)
                )
            }
            zrag_dsl::NameMatch::Ambiguous(ref ids) => {
                print!("{}", zrag_dsl::search_dep::render_candidates(&index, ids))
            }
            zrag_dsl::NameMatch::NotFound => {
    ...
    let (index, text_files) = zrag_dsl::build_index(&root_cow)?;
    tracing::info!(
    ...
    match command {
        ...
        DslCommands::ProjectMap {
            ...
            let kind_filter: Option<Vec<zrag_ts_core::types::Kind>> =
        ...
        DslCommands::SymbolBody { id } => {
            let entries = zrag_dsl::resolve_symbol_bodies(&index, &[id]);
            match entries.first() {
                Some(zrag_common::dsl::SymbolBodyEntry::Ok {
                    kind_short,
                    symbol_id,
                    start_line,
                    end_line,
                ...
                Some(zrag_common::dsl::SymbolBodyEntry::Err { message, .. }) => {
        ...
        DslCommands::SymbolBodies { ids } => {
            let entries = zrag_dsl::resolve_symbol_bodies(&index, &ids);
        ...
        DslCommands::SearchDep {
            ref name, depth, ..
        } => match zrag_dsl::resolve_name(&index, name) {
            zrag_dsl::NameMatch::Found(id) => {
                print!(
                    "{}",
                    zrag_dsl::render_symbol_overview(&index, id, depth, 6000)
        ...
        DslCommands::ChunkTrace => {
            ...
            let mut terminal_cache: HashMap<zrag_tree_sitter::Language, Vec<u16>> =
            ...
            let sizing = zrag_recursive_chunk::ChunkConfig {
                chunk_size: 2048,
                min_chunk_size: 512,
                chunk_overlap: 200,
            };
            ...
            for (i, file) in index.files.iter().enumerate() {
                ...
                for (ci, chunk) in chunks.iter().enumerate() {
                    ...
                    let sub = zrag_recursive_chunk::split_text(
                        &chunk.body,
                        &sizing,
                        Some(&ts_lang),
                        terminal_ids,
            ...
            for (ti, (path, contents)) in text_files.into_iter().enumerate() {
                ...
                for chunk in &chunks {
                    let c_start = Instant::now();
                    let sub = zrag_recursive_chunk::split_text(&chunk.body, &sizing, None, &[]);
                    let c_elapsed = c_start.elapsed();
