...
use zrag_protocol::request::DoctorReq;
use zrag_protocol::response::{CheckStatus, DoctorCheck, DoctorReport, Response};
...
pub async fn handle(req: &DoctorReq, state: &DaemonState) -> Response {
    ...
    let engine = if let Some(canon) = canonical_root.as_deref() {
        let pid = zrag_common::ids::project_id(canon);
    ...
    match zrag_common::paths::data_dir() {
        Ok(dir) => {
    ...
    if let Some(canon) = canonical_root.as_deref() {
        let pid = zrag_common::ids::project_id(canon);
        let root_str = canon.to_string_lossy();
        let db_path = match zrag_common::paths::project_dir(&pid) {
            Ok(p) => p.join("lance"),
            Err(e) => {
...
fn finalize(engine: &zrag_embed::AnyEmbedEngine, checks: Vec<DoctorCheck>) -> Response {
    let device = engine
        .hardware()
