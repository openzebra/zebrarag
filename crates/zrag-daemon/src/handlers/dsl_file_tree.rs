...
use zrag_protocol::request::FileTreeReq;
use zrag_protocol::response::{FileTreeBody, Response};
...
pub async fn handle(req: &FileTreeReq, state: &DaemonState) -> Response {
    ...
    let result = with_project(state, &req.project_root, |project| async move {
        ...
        Ok(FileTreeBody {
            text: zrag_dsl::render::dsl::render_files_only(&index, &file_indices),
        })
    })
    .await;
...
pub async fn ensure_dsl_index(
    ...
    root: &str,
) -> anyhow::Result<Arc<zrag_dsl::ProjectIndex>> {
    {
    ...
    let (idx_inner, _text_files) = zrag_dsl::build_index(root)?;
