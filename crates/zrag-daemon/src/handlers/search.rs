...
use tracing;
use zrag_protocol::request::{SearchMode, SearchReq};
use zrag_protocol::response::{Response, SearchHit, SearchResults};
use zrag_store::chunks_table::ChunkHit;
...
pub async fn handle(req: &SearchReq, state: &DaemonState) -> Response {
    let result = with_project(state, &req.project_root, |project| async move {
        let pid =
            zrag_common::ids::project_id(&std::path::Path::new(&req.project_root).canonicalize()?);
        ...
        if project_row.index_version < zrag_store::projects_table::INDEX_FORMAT_VERSION {
            anyhow::bail!(
                "project index is stale; run: zebrarag index -r {}",
                req.project_root
            );
        }
        ...
        let opts = zrag_pipeline::search::SearchOpts {
            limit: req.limit,
        ...
        let outcome = if req.exhaustive {
            zrag_pipeline::search::search_exhaustive(
                &req.query,
                &query_emb,
                &engine,
                &project.db,
...
fn dedup_overlapping_hits(hits: &mut Vec<zrag_pipeline::search::Hit>) {
...
mod tests {
    ...
    use zrag_common::{chunk_strategy::ChunkStrategy, file_type::FileType};
    use zrag_pipeline::search::Hit;
    use zrag_store::chunks_table::ChunkHit;
