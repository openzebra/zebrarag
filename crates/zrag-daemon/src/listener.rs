...
use zrag_protocol::codec::{read_frame, write_frame};
use zrag_protocol::request::Request;
...
async fn handle_request(req: Request, state: &DaemonState) -> Response {
    match req {
        ...
        Request::DslSearchDep(sd) => crate::handlers::dsl_search_dep::handle(&sd, state).await,
    }
}
