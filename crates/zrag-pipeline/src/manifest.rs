...
use zrag_store::FileRow;
use zrag_tree_sitter::{Language, detect_from_path};
...
fn load_pdf(path: &Path, rel: &str) -> Option<(String, [u8; 32], Option<Vec<PdfPageMeta>>)> {
    ...
    let pages = match zrag_pdf::extract_pages(&bytes) {
        Ok(p) => p,
        Err(e) => {
...
pub fn assemble_pdf_contents(pages: &[zrag_pdf::PageText]) -> (String, Vec<PdfPageMeta>) {
