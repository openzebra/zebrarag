...
use zrag_remote_embed::RemoteEmbedEngine;
...
impl AnyEmbedEngine {
    ...
    pub fn hardware(&self) -> Option<&zrag_hw::Hardware> {
        match self {
    ...
    pub fn device_with_hardware(&self, hardware: &zrag_hw::Hardware) -> Result<candle_core::Device> {
        match self {
            Self::Local(e) => e.device(),
            Self::Remote(_) => Ok(zrag_hw::candle_device(hardware)),
        }
    }
